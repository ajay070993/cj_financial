...
var dataTable = $('#datatable').DataTable({
    ...
    "language": {
      ...
      "ajax": {
          "url": "<?php echo base_url('dashboard/fetch_template_detail'); ?>",
          "type": "POST",
		  "data": function(data){
...
//workflow status
$(function () {
    $("#datatable_filter").append("<select class='form-control filter_select' id='workflow_filter' onchange='filterData1();'><option value='All'>All</option><option value='Completed'>Completed</option><option value='Spreading'>Spreading</option><option value='Qa'>Qa</option><option value='Rejected-downstream'>Rejected-downstream</option></select>");
})
...
$(document).on('click', '.assigned_to_me', function () {
    ...
    //return true;
    var surl = siteurl+'dashboard/assigned_case?history_id='+history_id;
    $.getJSON(surl,function(response){
        if (response.success) { 
            if(response.assigned) { 
            	$("#assigend_"+history_id).hide();
            	tdObj.closest('tr').find('td:nth-child(2)').html('<a href="<?php echo base_url('spread-detail/')?>'+history_id+'">'+tdObj.closest('tr').find('td:nth-child(2)').text()+'</a>');
